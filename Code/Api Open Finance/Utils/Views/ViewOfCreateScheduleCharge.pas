unit ViewOfCreateScheduleCharge;
...
type
  TViewOfCreateScheduleCharge = class(TForm)
    GroupBox3: TGroupBox;
...
procedure TViewOfCreateScheduleCharge.FormCreate(Sender: TObject);
begin
  Self.Position := poScreenCenter;
  InicializarComboBoxes;
end;

function TViewOfCreateScheduleCharge.GetIdemKey: String;
begin
  Result := txtIdemKey.Text;
end;

procedure TViewOfCreateScheduleCharge.InicializarComboBoxes;
begin
...
procedure TViewOfCreateScheduleCharge.btnCancelRequestClick(Sender: TObject);
begin
  ModalResult := mrCancel;
end;

procedure TViewOfCreateScheduleCharge.btnConfirmRequestClick(Sender: TObject);
var
  DocLen: Integer;
begin
...
function TViewOfCreateScheduleCharge.GetBody: String;
var
...
begin
  ...
  try
    ...
    ScheduleCharge.Pagador := Pagador;
    ...
    ScheduleCharge.Favorecido := Favorecido;
    ...
    Pagamento.dataAgendamento := Trim(txtDataAgendamento.Text);
    ...
    ScheduleCharge.Pagamento := Pagamento;
    ...
    if Trim(txtDataAgendamento.Text) = '' then
    begin
      if BodyRequest.Check('pagamento') and BodyRequest.O['pagamento'].Check('dataAgendamento') then
        BodyRequest.O['pagamento'].Remove('dataAgendamento');
    end;
...
procedure TViewOfCreateScheduleCharge.ClearRequestFields;
begin
  txtIdemKey.Text := '';
